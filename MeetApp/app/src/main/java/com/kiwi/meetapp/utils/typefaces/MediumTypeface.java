 ...
 */
public class MediumTypeface extends FontManager {
    @Override
    protected Typeface createTypeface(Context context, String type) {
        Typeface typeface;
        if(type.equals(FontManager.typeNormal))
            typeface = Typeface.createFromAsset(context.getAssets(), Fonts.MEDIUM);
        else if(type.equals(FontManager.typeItalic))
            typeface = Typeface.createFromAsset(context.getAssets(), Fonts.MEDIUM_ITALIC);
        else
            typeface = null;
        return typeface;
    }
